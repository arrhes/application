...
export function getToolCategory(toolName) {
    ...
    if (toolName === "process_array")
        return "Traitement de données";
    if (toolName === "read_all_years")
        return "Exercices fiscaux";
    if (toolName.includes("one_year") || toolName.startsWith("close_year") || toolName.startsWith("open_year")) {
        return "Exercice (général)";
    }
    if (toolName.includes("entry_line"))
        return "Lignes d'écriture";
    if (toolName.includes("entry_tag"))
        return "Étiquettes d'écriture";
    if (toolName.includes("entry") || toolName.includes("entries"))
    ...
    if (toolName.includes("tag"))
        return "Étiquettes";
    if (toolName.includes("balance_sheet"))
        return "Bilan";
    if (toolName.includes("income_statement"))
        return "Compte de résultat";
    if (toolName.includes("computation"))
        return "Calculs et rubriques";
    if (toolName.includes("file"))
        return "Fichiers";
    if (toolName.includes("folder"))
        return "Dossiers";
    if (toolName.includes("document") || toolName.includes("report"))
        return "Rapports";
    return "Autres";
}
...
export default function Content() {
    ...
    const groupedTools = useMemo(() => {
        ...
        for (const [key, tools] of grouped) {
            grouped.set(key, tools.toSorted((a, b) => a.labelFr.localeCompare(b.labelFr, "fr")));
        }
        return [
            ...grouped.entries(),
        ].toSorted((a, b) => {
    ...
    return (<>
            <DocHeader title="Outils" description={`Liste complète des ${totalToolCount} outils disponibles pour l'assistant comptable.`}/>
            ...
            marginBottom: "1rem",
                ...
                <input type="text" aria-label="Rechercher un outil" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher un outil..." className={css({
            width: "100%",
            maxWidth: "24rem",
            ...
            gap: "1.5rem",
                ...
                {groupedTools.map(([category, tools]) => (<div key={category} className={css({
                display: "flex",
                flexDirection: "column",
                gap: "0.5rem",
            })}>
                ...
                borderBottomColor: "neutral/10",
                            ...
                            {tools.map((tool) => (<div key={tool.name} className={css({
                    border: "1px solid",
                ...
                {groupedTools.length === 0 && (<p className={css({
                fontSize: "sm",
                color: "neutral/60",
                margin: 0,
            })}>
