...
export default function Content() {
    ...
    return (<>
            <DocHeader title="Glossaire comptable" description="Tous les termes essentiels de la comptabilité en un coup d'oeil"/>
            ...
            pointerEvents: "none",
                ...
                <input type="text" aria-label="Rechercher un terme" placeholder="Rechercher un terme..." value={query} onChange={(e) => setQuery(e.target.value)} className={css({
            width: "100%",
            ...
            {isSearching ? (<div className={css({
                ...
                fontWeight: "medium",
                    ...
                    {filteredTerms.map((term) => (<GlossaryListItem key={term.slug} term={term.term} englishTranslation={term.englishTranslation} slug={term.slug} definition={term.definition}/>))}
                    {filteredTerms.length === 0 && (<p className={css({
                    fontSize: "sm",
                    color: "neutral/50",
                    padding: "2rem 0",
                    textAlign: "center",
        ...
        <div className={css({
                ...
                gap: "1.5rem",
                    ...
                    {Array.from(termsByLetter.entries()).map(([letter, terms]) => (<div key={letter} className={css({
                    display: "flex",
                    flexDirection: "column",
                    gap: "0.5rem",
                })}>
                    ...
                    borderBottomColor: "neutral/10",
                            ...
                            {terms.map((term) => (<GlossaryListItem key={term.slug} term={term.term} englishTranslation={term.englishTranslation} slug={term.slug} definition={term.definition}/>))}
                        </div>))}
                </div>)}
        </>);
