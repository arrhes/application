...
export function Account4458AccountingDocPage() {
    const entry = getAccountBySlug("4458")
    if (!entry) return null

    const parentAccount = getAccount("445")
    const children = getDirectChildren("4458")
    ...
    return (
        <DocRoot>
            ...
            <DebitCreditSection entry={entry} debitMeaning="Variable" creditMeaning="Variable" />

            {parentAccount && <ParentAccountSection parentAccount={parentAccount} />}

            <ChildrenListSection children={children} />

            <PracticalUsageSection entry={entry} debitMeaning="Variable" creditMeaning="Variable" />
