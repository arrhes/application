...
function DiffTag(props: { label: string; color: "error" | "success" | "information" }) {
...
function AccountRow(props: { account: AccountEntry }) {
    ...
    const isRemoved = account.removed === true
    const isAdded = account.added === true
    const isModified = account.previousLabel !== undefined || account.previousSystem !== undefined
    return (
        <div style={{ paddingLeft: `${depth * 1}rem` }}>
            <LinkButton
                ...
                className={css({
                    ...
                    cursor: "pointer",
                    width: "100%",
                    ...(isRemoved && { opacity: 0.55 }),
                })}
            >
                <span
                    className={css({
                        fontWeight: "bold",
                        color: isRemoved ? "error" : "primary",
                        fontFamily: "mono",
                        minWidth: "3.5rem",
                ...
                <span
                    className={css({
                        ...
                        whiteSpace: "nowrap",
                        fontStyle: isFacultatif ? "italic" : "normal",
                        color: isRemoved ? "error/70" : isFacultatif ? "neutral/50" : "neutral",
                        ...(isRemoved && { textDecoration: "line-through" }),
                    })}
                >
                    {account.label}
                </span>
                {isRemoved && <DiffTag label="Supprimé" color="error" />}
                {isAdded && <DiffTag label="Nouveau" color="success" />}
                {isModified && !isRemoved && !isAdded && <DiffTag label="Modifié" color="information" />}
            </LinkButton>
        </div>
    )
}
