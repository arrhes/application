...
export function UblInvoiceViewer(props: {
    ...
    const prettyXml = useMemo(() => createPrettyXml(props.xmlContent), [props.xmlContent])

    function downloadXml() {
        const blob = new Blob([props.xmlContent], { type: "application/xml;charset=utf-8" })
        const objectUrl = URL.createObjectURL(blob)
        const link = document.createElement("a")
        link.href = objectUrl
        link.download = props.downloadFileName ?? "invoice.xml"
        ...
        >
            ...
            >
                ...
                <div className={css({ display: "flex", alignItems: "center", gap: "0.5rem" })}>
                    ...
                    <Button onClick={() => setShowRawXml((state) => !state)}>
                        <ButtonOutlineContent text={showRawXml ? "Vue lisible" : "XML brut"} />
                    </Button>
                    <Button onClick={downloadXml}>
                        <ButtonOutlineContent leftIcon={<IconDownload />} text="Télécharger XML" />
                    </Button>
                </div>
            </div>
            ...
            {showRawXml ? (
                ...
                >
                    ...
                    >
                        ...
                        <p>Montant HT: {parsed.amountHT || "-"}</p>
                        <p>Montant TVA: {parsed.amountTVA || "-"}</p>
                        <p>Montant TTC: {parsed.amountTTC || "-"}</p>
                    </div>
                        ...
                        <table className={css({ width: "100%", borderCollapse: "collapse", fontSize: "sm" })}>
                            <thead>
                                <tr className={css({ borderBottom: "1px solid token(colors.neutral/15)" })}>
                                    <th className={css({ textAlign: "left", padding: "0.6rem" })}>#</th>
                                    <th className={css({ textAlign: "left", padding: "0.6rem" })}>Service</th>
                                    <th className={css({ textAlign: "right", padding: "0.6rem" })}>Quantité</th>
                                    <th className={css({ textAlign: "right", padding: "0.6rem" })}>Montant HT</th>
                                    <th className={css({ textAlign: "right", padding: "0.6rem" })}>TVA</th>
                                    <th className={css({ textAlign: "right", padding: "0.6rem" })}>Montant TTC</th>
                                </tr>
                            </thead>
                            <tbody>
