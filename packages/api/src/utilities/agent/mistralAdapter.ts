...
/**
 * Minimum time between consecutive LLM API calls (ms).
 * Prevents rapid-fire calls from triggering Mistral's rate limiter.
 */
const MIN_CALL_INTERVAL_MS = 500

/**
 * Track the last API call timestamp globally across all adapter instances.
 * This prevents rapid succession calls from router → executor → tool loop.
 */
let lastApiCallTimestamp = 0

/**
 * Extracts the retry delay from a 429 error.
...
/**
 * Ensure a minimum interval between LLM API calls.
...
/**
 * Mistral adapter using OpenAI SDK's Chat Completions API.
 *
 * The @tanstack/ai-openai adapter (v0.7+) uses OpenAI's Responses API
 * (`/v1/responses`), which is OpenAI-specific and not supported by Mistral
 * or other OpenAI-compatible providers.
 *
 * This adapter uses the legacy Chat Completions API (`/v1/chat/completions`),
 * which Mistral's OpenAI-compatible endpoint supports.
 */
...
> {
    ...
    async *chatStream(options: TextOptions): AsyncIterable<StreamChunk> {
        ...
        try {
            ...
            for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
                try {
                    ...
                    if (isRateLimitError(retryError) && attempt < MAX_RETRIES) {
                        ...
                        console.warn(
                            `[MistralChatAdapter] Rate limited (429), retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`,
                        )
                        await sleep(delay)
                        continue
                    }
            ...
            if (!stream) {
                throw new Error("Failed to create chat completion stream after retries")
            }
            ...
            console.error("[MistralChatAdapter] Stream error:", err.message)
            yield {
                type: "RUN_ERROR",
                runId,
                model: options.model,
    ...
    async structuredOutput(
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                if (isRateLimitError(retryError) && attempt < MAX_RETRIES) {
                    const delay = getRetryDelay(retryError, attempt)
                    console.warn(
                        `[MistralChatAdapter] Rate limited (429) in structuredOutput, retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`,
                    )
                    await sleep(delay)
                    continue
                }
        ...
        if (!response) {
            throw new Error("Failed to create structured output after retries")
        }
...
/**
 * Creates a Mistral chat adapter using OpenAI-compatible Chat Completions API.
 */
