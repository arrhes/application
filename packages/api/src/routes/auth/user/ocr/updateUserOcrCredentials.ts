...
export const updateUserOcrCredentialsRoute = registerRoute(updateUserOcrCredentialsRouteDefinition, async (c) => {
    const { user } = await checkAuthMiddleware({
        context: c,
    })
    const body = await validateBodyMiddleware({
        context: c,
        schema: updateUserOcrCredentialsRouteDefinition.schemas.body,
    })
    ...
    return response({
        context: c,
        statusCode: 200,
        schema: updateUserOcrCredentialsRouteDefinition.schemas.return,
        data: {
            success: true,
        },
    })
